...
class SkinCareExpertSystem(KnowledgeEngine):
    ...
    def declare_user_facts(self, user_data):
        for key, value in user_data.items():
            # Assuming the keys in user_data correspond to fields in User Fact
            self.declare(Fact(ident=key, text=value))
...
def index():
    ...
    first_question = expert_system._initial_action()
...
def question(question_id):
    ...
    if request.method == 'POST':
        ...
        cf = request.form['cf']
        engine.declare(Answer(ident=ident, text=text, cf=cf))

    engine.run()

    questions = list(engine.facts.values())
    questions = [q for q in questions if isinstance(q, Question)]
